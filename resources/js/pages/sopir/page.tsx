...
export default function SopirPage() {
  ...
  const openAdd = () => {
    setEditing(null)
    setForm({ id_sopir: "", nama: "", nip: "", no_hp: "", status: "", sim: "", masa_berlaku_sim: "", foto: "" })
    setOpen(true)
  }
  ...
  const onSubmit = () => {
    if (editing) {
      ...
      setData((prev) => prev.map((r) => (r.id_sopir === editing.id_sopir ? form : r)))
      toast({ title: "Data berhasil disimpan", description: "Perubahan sopir telah disimpan." })
    } else {
      console.log("[CREATE SOPIR]", form)
      setData((prev) => [
        { ...form, id_sopir: form.id_sopir || `S-${String(prev.length + 1).padStart(3, "0")}` },
        ...prev,
      ])
      toast({ title: "Data berhasil disimpan", description: "Sopir baru telah ditambahkan." })
    }
    setOpen(false)
  }
  ...
  const onDelete = (row: Sopir) => {
    ...
    toast({ title: "Data berhasil dihapus", description: `Sopir ${row.id_sopir} dihapus.` })
  }
  ...
  return (
    <div className="space-y-4">
              ...
              <TableBody>
                {data.map((row) => (
                  <TableRow key={row.id_sopir}>
                    <TableCell>{row.id_sopir}</TableCell>
                    <TableCell>{row.nama}</TableCell>
                    <TableCell>{row.nip}</TableCell>
                    <TableCell>{row.no_hp}</TableCell>
                    <TableCell>{row.status}</TableCell>
                    <TableCell>{row.sim}</TableCell>
                    <TableCell>{row.masa_berlaku_sim}</TableCell>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Tindakan ini tidak dapat dibatalkan. Sopir {row.id_sopir} akan dihapus.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
      ...
      <FormModal
        title={editing ? `Ubah Sopir ${editing.id_sopir}` : "Tambah Sopir"}
        open={open}
        onOpenChange={setOpen}
        onSubmit={onSubmit}
      >
        <div className="grid md:grid-cols-2 gap-4">
          <div className="grid gap-2">{/* Placeholder for Label and Input components */}</div>
